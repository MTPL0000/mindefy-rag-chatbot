...
export default function ChatPage() {
  ...
  const isWelcomeScreen = currentChat.length <= 1 && currentChat[0]?.role === "assistant";
    ...
    <ProtectedRoute>
      <div className="h-screen flex flex-col bg-gray-50">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className={`flex items-center justify-between w-full xl:max-w-6xl 2xl:max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 xl:px-8 bg-gray-50 ${isWelcomeScreen ? 'py-4 mt-4' : 'py-2 mt-3'}`}>
            {/* Left: Model Dropdown */}
            <div ref={dropdownRef} className="flex-shrink-0">
              <div className="relative">
                <button
                  onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                  className="text-xs sm:text-sm px-3 py-2 sm:px-4 sm:py-2 border border-gray-300 rounded-full bg-white text-gray-700 focus:outline-none hover:border-gray-400 hover:shadow-sm transition-all duration-200 flex items-center gap-2 shadow-sm"
                >
                  <span className="truncate text-xs sm:text-sm font-medium">{getSelectedModelLabel()}</span>
                  <svg 
                ...
                {/* Dropdown Menu */}
                {isDropdownOpen && (
                  <div className="absolute top-full left-0 mt-1.5 min-w-full bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden z-50">
                    {models.map((model) => (
                      <button
                        key={model.value}
                        onClick={() => handleModelSelect(model.value)}
                        className={`w-full text-left px-3 py-2 text-xs sm:text-sm hover:bg-gray-50 transition-colors duration-150 flex items-center justify-between whitespace-nowrap ${
            ...
            <button
              ...
              className="cursor-pointer flex items-center gap-1.5 px-3 py-2 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium transition-all duration-200 border border-gray-300 bg-white text-gray-700 hover:border-gray-400 hover:shadow-sm shadow-sm"
            >
          ...
          <div className="flex-1 overflow-y-auto min-h-0">
            {isWelcomeScreen ? (
              ...
              <div className="flex flex-col items-center justify-center min-h-full px-4 py-4 sm:py-6 md:py-8">
                {/* Sparkle Icon */}
                <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center mb-4 sm:mb-6" style={{backgroundColor: '#332771'}}>
                ...
                <p className="text-gray-600 text-center max-w-md sm:max-w-lg md:max-w-xl mb-6 sm:mb-8 md:mb-10 text-xs sm:text-sm md:text-base px-2">
                  I can answer questions based on your uploaded documents. Ask me anything about your knowledge base.
                </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 w-full max-w-sm sm:max-w-lg md:max-w-2xl px-2 sm:px-4">
                  {sampleQuestions.map((item, index) => (
                    <button
                      ...
                      className="flex flex-col items-start p-3 sm:p-4 md:p-5 bg-white rounded-xl sm:rounded-2xl border border-gray-100 shadow-md hover:border-gray-200 hover:shadow-xl transition-all duration-200 text-left group"
                    >
                      <div className="w-9 h-9 sm:w-10 sm:h-10 md:w-11 md:h-11 rounded-lg sm:rounded-xl flex items-center justify-center mb-2 sm:mb-3 md:mb-4" style={{backgroundColor: 'rgba(51, 39, 113, 0.08)'}}>
                        <item.icon className="w-4 h-4 sm:w-5 sm:h-5" style={{color: '#332771'}} />
                      </div>
                      <h3 className="font-semibold text-gray-900 mb-1 sm:mb-1.5 text-sm sm:text-base">{item.title}</h3>
                      <p className="text-xs sm:text-sm text-gray-500 leading-relaxed">{item.question}</p>
                    </button>
                  ))}
                </div>
              </div>
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          color: message.role === "user" ? '#332771' : '#374151',
                        ...
                        {message.role === "assistant" && !message.isError && (
                          <div className="flex items-center space-x-1 mt-2 ml-1">
                            <button
                              ...
                              className="p-1.5 rounded hover:bg-gray-200/50 transition-colors"
                              title="Copy response"
                            >
                            ...
                            >
                              <ThumbsUp 
                                ...
                                style={{
                                  color: feedbackState[message.id] === 'good' ? '#10b981' : '#9ca3af',
                                  fill: feedbackState[message.id] === 'good' ? '#10b981' : 'none'
                                }}
                              />
                            </button>
                            <button
                            ...
                            >
                              <ThumbsDown 
                                className="w-4 h-4 transition-colors" 
                                style={{
                                  color: feedbackState[message.id] === 'bad' ? '#ef4444' : '#9ca3af',
                                  fill: feedbackState[message.id] === 'bad' ? '#ef4444' : 'none'
                                }}
                              />
                            </button>
                          </div>
                      ...
                      <div className="rounded-lg px-4 py-3">
                        <div className="flex space-x-1">
                          <div
                            ...
                            style={{ animationDelay: "0ms", backgroundColor: '#ebdad4' }}
                          ></div>
                          <div
                            className="w-2 h-2 rounded-full animate-bounce"
                            style={{ animationDelay: "150ms", backgroundColor: '#d4c4be' }}
                          ></div>
                          <div
                            className="w-2 h-2 rounded-full animate-bounce"
                            style={{ animationDelay: "300ms", backgroundColor: '#332771' }}
                          ></div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="flex items-end space-x-2 sm:space-x-3">
              <div className="relative w-full max-w-3xl mx-auto">
                ...
                <button
                  ...
                  className="absolute cursor-pointer right-2 sm:right-2.5 top-1/2 -translate-y-1/2 p-2 sm:p-2.5 rounded-full transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105 backdrop-blur-sm border border-white/20"
                  style={{
                    backgroundColor: input.trim() && !isLoadingChat ? '#332771' : '#f3f4f6',
                    color: input.trim() && !isLoadingChat ? 'white' : '#9ca3af',
                    cursor: input.trim() && !isLoadingChat ? 'pointer' : 'not-allowed',
                    boxShadow: input.trim() && !isLoadingChat ? '0 2px 8px rgba(51, 39, 113, 0.2)' : 'none'
                  }}
                  onMouseEnter={(e) => {
